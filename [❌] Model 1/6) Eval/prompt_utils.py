...
def build_structured_prompt(user_parsed: dict, mask_instruments: bool = True):
    ...
    in the format:
    <|startofpiece|><NAME=...><BPM=...><DURATION_BEATS=...><DURATION_MINUTES=...><TRACKS><MASK> <TRACKSEP> Instrument: <MASK><|endofpiece|>
    ...
    name = user_parsed.get("name", "Untitled")
    bpm = float(user_parsed.get("bpm", 120.0))
    ...
    if prompt_tracks:
        ...
        for inst in instruments:
            if mask_instruments:
                lines.append(f"{inst.capitalize()}: <MASK>")
            else:
                lines.append(f"{inst.capitalize()}: ")
    ...
    prompt = (
        ...
        f"<TRACKS>{tracks_snippet}<|endofpiece|>"
    )
