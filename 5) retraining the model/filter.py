...
def is_musical_token(token):
    """Check if token is a musical token (updated for subword tokens)"""
    # Full musical tokens (if they survive tokenization)
    full_pattern = r'^[A-G][#b]?[0-9]+_[a-z0-9/\.]+$|^Rest_[a-z0-9/\.]+$'
    
    # Subword components that are part of musical tokens
    note_parts = ['C', 'D', 'E', 'F', 'G', 'A', 'B', '#', 'b']
    duration_parts = ['_q', '_e', '_h', '_w', '_s',
                    '_0', '_1', '_2', '_3', '_4', '_5', '_6', '_7', '_8', '_9',
                    '_0.25', '_0.5', '_0.75', '_1.0', '_1.5', '_2.0',
                    '_1/4', '_1/8', '_1/16', '_3/4', '_3/8', '_1/3', '_2/3', '_1/6']
    rest_parts = ['Rest', 'rest', 'REST']
    
    # Special tokens
    special_tokens = ['<|startofpiece|>', '<|endofpiece|>', '<TRACKS>', '<TRACKSEP>', 
                        '<NAME=', '<BPM=', '<DURATION_BEATS=', '<DURATION_MINUTES=', '<MASK>']
    
    return (re.match(full_pattern, token) or
...
def add_musical_tokens_to_tokenizer(tokenizer):
    ...
    notes = ['C', 'D', 'E', 'F', 'G', 'A', 'B']
    duartions = ['q', 'e', 'h', 'w', 's', '0.25', '0.5', '0.75', '1.0', '1.5', '2.0',
                '1/4', '1/8', '1/16', '3/4', '3/8', '1/3', '2/3', '1/6']
    
    # Notes with durations
    for note in notes:
        for octave in range(1, 8):
            for duration in duartions:
                musical_tokens.append(f"{note}{octave}_{duration}")
                musical_tokens.append(f"{note}#{octave}_{duration}")
                musical_tokens.append(f"{note}b{octave}_{duration}")
    
    # Rests with durations
    for duration in duartions:
        musical_tokens.append(f"Rest_{duration}")
