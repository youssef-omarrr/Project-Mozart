...
def sine_positional_encoding(max_len, embed_dim):
    ...
    # will later hold sine and cosine values for each position.
    pos_encoding = torch.zeros(max_len, embed_dim)

    # Create a column vector representing token positions [1, 2, .., max_len-1]
    positions_vector = torch.arange(0, max_len, dtype=torch.float).unsqueeze(
        1)  # shape after unsqueezing: (1, max_len)
...
class ProjectMozart(nn.Module):
    def __init__(self,
        ...
        # fully connected layer: maps each token’s hidden vector (embed_dim) to the output space (vocab_size)
